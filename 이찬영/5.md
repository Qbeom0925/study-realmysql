...
- 갭 락은 자체로 보다 넥스크 키 락의 일부로 자주 사용된다.
  ...
  > 근데 트랜잭션A의 실행중 트랜잭션B에서 age가 15인 레코드가 하나 추가되었다. 이때 트랜잭션A의 결과가 16개로 나온다면 이것은 Phantom rows가 발생한 것이다.  
...
- 더티 리드 현상은 데이터가 나타났다가 사라졌다 하는 현상을 초해하므로 어플리케이션 개발자와 사용자를 상당히 혼란스럽게 한다.
...
- 즉 사용자 B의 하나의 트랜잭션 내에서 똑같은 쿼리를 SELECT 실했을 때는 항상 같은 결과를 반환해야 한다는 "REPEATABLE READ" 정합성에 어긋나는 것이다.

  > Q : 근데 그렇다면 이때는 왜 언두 로그에서 데이터를 안가져옴??  
  > A : Commit이 되었기 때문에! Commit이 되지 않았다면 Undo영역의 데이터를 가져오겠지만 Commit되었기 때문에 테이블에서 데이터를 가져온다.

- 이와 같은 부정합 문제는 일반적인 웹 프로그램에서는 크게 문제가 되지 않지만 하나의 트랜잭션에서 동일 데이터를 여러 번 읽고 변경하는 작업이 금전적인 처리와 연결되면 연결되면 문제가 될 수 있다.
...
- 물론 가자 오래된 트랜잭션 번호 이전의 트랜잭션에 의해 변경된 모든 언두 데이터가 필요한 것은 아니다. 더 정확히는 특정 트랜잭션 번호의 구간 내에서 백업된 언두 데이터가 보존되어야 한다.
...
- 하지만 사용자B가 실행하는 두번의 SELECT ... FOR UPDATE 쿼리 결과는 서로 다른다.
